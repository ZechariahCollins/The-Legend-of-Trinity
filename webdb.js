...
async function multiplayerInit()
{
    web_socket = new WebSocket("ws://localhost:8082");
    web_socket.onopen= function(event) {
        web_socket.send("We are connected!");
    };
    web_socket.addEventListener("message", ({ data }) => {
	split_data = data.split(",");
	final_data = split_data.slice(1, split_data.length).join();
	if (split_data[0].toString() == "id") {
	    player_id = final_data[0];
	} else {
	    server_data = final_data.valueOf();
	}
    });
    window.foo("Did it work? :o");
}

async function getServerUpdates(playerPos)
{
    web_socket.send(playerPos);
}

async function getServerData(d)
{
    console.log(d);
    return 2;
    //return server_data;
