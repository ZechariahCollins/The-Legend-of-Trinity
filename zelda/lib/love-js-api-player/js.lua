...
local function retrieveJS(funcToCall, id)
    ...
    JS.callJS("FS.writeFile('/__test"..id.."', "..funcToCall..");")
end
...
function _Request:new(isPromise, command, onDataLoaded, onError, timeout, id)
	...
	if (not love.filesystem.getInfo(love.filesystem.getSaveDirectory().."/__temp"..self.id)) then
	    print("File not created yet: "..love.filesystem.getSaveDirectory().."/__temp"..self.id)
	    return nil
	end
...
function JS.newRequest(funcToCall, onDataLoaded, onError, timeout, optionalId)
    ...
    table.insert(__requestQueue, _Request:new(false, funcToCall, onDataLoaded, onError, timeout or 100, optionalId or _requestCount))
end
